...
class EpochBasedTrainLoop(BaseLoop):
    ...
    def run(self) -> torch.nn.Module:
        """Launch training."""
        if self.runner.rank == 0 and wandb.run is None and os.environ.get('WANDB_MODE') != 'disabled':
            wandb.init(
                project="ForestMamba",
                name=os.path.basename(self.runner.work_dir),
                resume="allow",
            )
    ...
    def run_epoch(self) -> None:
        ...
        epoch_losses = {}
        pbar = tqdm(self.dataloader, desc=f'Epoch [{self._epoch + 1}/{self._max_epochs}]',
        ...
        if self.runner.rank == 0 and wandb.run is not None and epoch_losses:
            log_dict = {'train/' + k: sum(vs) / len(vs) for k, vs in epoch_losses.items()}
...
class ValLoop(BaseLoop):
    ...
    def run(self) -> dict:
        ...
        if self.runner.rank == 0 and wandb.run is not None and metrics:
            train_loop = getattr(self.runner, 'train_loop', None)
            epoch = getattr(train_loop, '_epoch', 0) if train_loop else 0
            wandb_metrics = {'val/' + k: v for k, v in metrics.items()}
