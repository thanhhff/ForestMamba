...
model = dict(
    type='ForAINetV2OneFormer3D_CHMquery_Spatial',
    ...
    radius=radius,
    score_th=score_th,
    # ── CHM query initialisation (identical to v2) ─────────────────────────
    chm_resolution=0.5,
    chm_min_height=1.5,
    chm_allometric_a=0.25,
    chm_allometric_b=0.5,
    chm_pool_radius=1.5,
    # ── Backbone: bidirectional SparseMambaEncoder (identical to v2) ───────
    backbone=dict(
        ...
        expand=2,
        slab_thickness=5),
    # ── Decoder: SSM query decoder (LaSSM-inspired, replaces cross-attention)
    # Cross-attention → SSMCrossModalLayer  (bidirectional scan over
    #   [backbone_feats ‖ queries], giving every query full-scene context)
    # Self-attention  → SSMSelfLayer        (bidirectional scan over queries)
    decoder=dict(
        ...
        in_channels=32,
        d_model=256,
        d_state=64,      # SSM state dimension (larger than backbone's 16)
        d_conv=4,
        expand=2,
        hidden_dim=1024,
        dropout=0.0,
