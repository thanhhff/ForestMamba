...
model = dict(
    type='ForAINetV2OneFormer3D_CHMquery_MultiScale',
    ...
    radius=radius,
    score_th=score_th,
    # ── Multi-scale CHM query initialisation ──────────────────────────────
    # Fine grid catches trees in dense stands; coarse grid catches large crowns.
    # Spatial NMS merges duplicates: keep the taller peak when two peaks are
    # within chm_min_peak_dist metres of each other.
    chm_resolution_fine=0.3,
    chm_resolution_coarse=0.7,
    chm_min_peak_dist=1.0,
    chm_min_height=1.5,
    chm_allometric_a=0.25,
    chm_allometric_b=0.5,
    chm_pool_radius=1.5,
    # ── Backbone: SparseMambaEncoder ─────────────────────────────────────
    # bidirectional=True  → fwd + bwd Mamba per level (default)
    # bidirectional=False → fwd only (fewer params, faster)
    backbone=dict(
        ...
        slab_thickness=5,
        bidirectional=True),
    # ── Decoder: SSM query decoder without KNN aggregation (ablation D) ──
    decoder=dict(
        type='ForAINetv2SSMQueryDecoder_XAwarequery',
        num_layers=6,
        num_classes=1,
